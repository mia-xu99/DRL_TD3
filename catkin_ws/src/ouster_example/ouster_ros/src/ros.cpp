...
void fliter_to_cloud(ouster_ros::Cloud& cloud, ouster_ros::Cloud& out, ouster_ros::Cloud& out2, int thred, int window) {
    out.clear();
    out2.clear();
    out.resize(cloud.size());
    out2.resize(cloud.size());
    ...
    for (auto u = 0; u < h ; u++) {
        for (auto v = 0; v < w; v++) {
            ...
            auto p = cloud(v, u);
            bool flag = true;
            for (int i = -window; i <= window && flag; i++) {
                for (int j = -window; j <= window && flag; j++) {
                    if (i == 0 && j == 0) {
                        continue;
                    }
                    auto row = u + i, col = v + j;
                    if (row < 0 || col < 0 || row >= h || col >= w) {
                        continue;
                    }

                    if (cloud(col, row).range < 300) {
                        continue;
                    }
                    // ROS_INFO("row: %d ,%d", row, col);

                    int32_t diff = p.range - cloud(col, row).range;
                    // diff = abs(diff);

                    if (diff >= thred) {
                    // ROS_INFO("diff: %d t : %d,  cur :%d, vs:%d", diff, thred, p.range, cloud(col, row).range);
...
void fliter_to_cloud(ouster_ros::Cloud& cloud) {

    for (auto u = 63; u < 64; u++) {
        for (uint32_t v = 0; v < cloud.width; v++) {
            // 过滤gps干扰
            // if (v >= ls.w * 0.7 && v <= ls.w * 0.8 && u > 42 ) {
            //     continue;
            // }
            auto front = cloud(v, u), back = cloud(v, u-1);
            float zdiff = abs(back.z - front.z);
            if (zdiff > 0.075 || (front.z >= -0.75)) {
                continue;
            }
            uint32_t rangeDiff = back.range - front.range;
            if (rangeDiff > 50) {
                cloud(v, u).x = cloud(v, u).x * 3;
                cloud(v, u).y = cloud(v, u).y * 3;
                cloud(v, u).z = -0.5;
                cloud(v, u).reflectivity = 9999;
...
sensor_msgs::PointCloud2 cloud_to_cloud_msg(const Cloud& cloud, ns timestamp,
    ...
    // pcl::PassThrough<pcl::PointCloud<Point> > pass;
    ...
    // msg.header.stamp.fromNSec(ros::Time::now());
